  ...
  return (
    <nav className="fixed top-0 inset-x-0 z-50">
      <div className={`w-full mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between 
        ...
        supports-[backdrop-filter]:bg-white/10 supports-[backdrop-filter]:backdrop-blur-md supports-[backdrop-filter]:backdrop-saturate-150
        ${scrolled ? 'bg-slate-950/95 shadow-md ring-1 ring-white/10' : 'shadow-sm'}`}>
        <a href="#intro" className="flex items-center gap-2 font-semibold text-slate-100">
        ...
        <button
          ...
          className="md:hidden inline-flex items-center justify-center rounded-lg px-3 py-2 text-slate-200 focus:outline-2 focus:outline-blue-700"
        >
      ...
      {open && (
        <div className="md:hidden fixed inset-0 bg-black/40" onClick={() => setOpen(false)}>
          <div
            className="ml-auto h-full w-80 bg-slate-950 text-slate-100 shadow-lg outline-none"
            role="dialog"
            aria-modal="true"
